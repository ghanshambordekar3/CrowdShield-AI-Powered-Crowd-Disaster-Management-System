...
@RestController
@CrossOrigin(origins = "https://crowd-shield-ai-powered-crowd-disas-eosin.vercel.app/")
...
public class SmsController {
    ...
    public ResponseEntity<Map<String, Object>> sendSms(@RequestBody Map<String, Object> smsRequest) {
        ...
        try {
            // Read phone number from request if provided; otherwise fall back to demo number
            String rawPhone = (String) smsRequest.getOrDefault("phoneNumber", "XXXXXXXXXX");
            String message = (String) smsRequest.get("message");
            Integer zoneId = smsRequest.get("zoneId") != null ? Integer.valueOf(smsRequest.get("zoneId").toString()) : null;
            Double latitude = smsRequest.get("latitude") != null ? Double.valueOf(smsRequest.get("latitude").toString()) : null;
            Double longitude = smsRequest.get("longitude") != null ? Double.valueOf(smsRequest.get("longitude").toString()) : null;
            ...
            if (rawPhone == null) rawPhone = "XXXXXXXXXX";
            ...
            System.out.println("SmsController: rawPhone='" + rawPhone + "' digits='" + digits + "' e164='" + e164 + "'");
            ...
            Integer zoneId = smsRequest.get("zoneId") != null ? Integer.valueOf(smsRequest.get("zoneId").toString()) : null;
            Double lat = smsRequest.get("latitude") != null ? Double.valueOf(smsRequest.get("latitude").toString()) : null;
            Double lng = smsRequest.get("longitude") != null ? Double.valueOf(smsRequest.get("longitude").toString()) : null;
    ...
    private boolean sendViaTextBelt(String message, String e164Number, String apiKey) {
        try {
            ...
            String phoneNumber = e164Number.startsWith("+91") ? e164Number.substring(3) : e164Number.replaceAll("\\D", "");
    ...
    public ResponseEntity<Map<String, Object>> sendLocationAlert(@RequestBody Map<String, Object> alertRequest) {
        ...
        try {
            ...
            Integer zoneId = alertRequest.get("zoneId") != null ? Integer.valueOf(alertRequest.get("zoneId").toString()) : null;
            Double latitude = alertRequest.get("latitude") != null ? Double.valueOf(alertRequest.get("latitude").toString()) : null;
            Double longitude = alertRequest.get("longitude") != null ? Double.valueOf(alertRequest.get("longitude").toString()) : null;
    ...
    private String generateAlertMessage(String alertType, Integer zoneId, Double latitude, Double longitude) {
        switch (alertType != null ? alertType.toUpperCase() : "GENERAL") {
            case "FIRE":
                return updateMessageWithLocation("🔥 FIRE EMERGENCY\n\nImmediate evacuation required at [LOCATION]! Move to nearest safe exit.", zoneId, latitude, longitude);
            case "MEDICAL":
                return updateMessageWithLocation("⚕️ MEDICAL EMERGENCY\n\nMedical assistance required at [LOCATION]. Emergency services notified.", zoneId, latitude, longitude);
            case "OVERCROWDING":
                return updateMessageWithLocation("👥 OVERCROWDING ALERT\n\nDangerous crowd levels at [LOCATION]. Avoid area, use alternate routes.", zoneId, latitude, longitude);
            case "STAMPEDE":
                return updateMessageWithLocation("🏃♂️ STAMPEDE RISK\n\nHigh risk at [LOCATION]! STOP MOVING. Stay calm, wait for crowd to thin.", zoneId, latitude, longitude);
            default:
                return updateMessageWithLocation("⚠️ EMERGENCY ALERT\n\nEmergency at [LOCATION]. Follow safety instructions.", zoneId, latitude, longitude);
        }
    }
